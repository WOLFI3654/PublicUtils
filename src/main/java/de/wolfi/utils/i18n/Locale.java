...
public class Locale
{
    ...
    public synchronized void loadLocaleDataFiles(File root, List<String> langs)
    {
        this.properties.clear();

        for (String s : langs)
        {
            String langFile = String.format("%s.lang", new Object[] {s});

            for (File category : root.listFiles())
            {
                try
                {
                    this.loadLocaleData(new File(category, langFile));
                }
     ...
     */
    private void loadLocaleData(File lang) throws IOException
    {
        System.out.println("Loading lang: "+lang.getPath());
        {
